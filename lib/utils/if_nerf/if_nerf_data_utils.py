...
def get_near_far(bounds, ray_o, ray_d):
    ...
    # 使用三维边界框计算交点
    bounds = bounds + np.array([-0.01, 0.01])[:, None]          # [2, 3] + [2, 1]
    # min-0.01 , max+0.01
    nominator = bounds[None] - ray_o[:, None]   # [1, 2, 3] - [1024, 1, 3] = nominator.shape : (1024, 2, 3)     1024条光线起始点和边界（最远和最近）点形成的线段
    
    # calculate the step of intersections at six planes of the 3d bounding box
    # 3d边界框六个平面处的交点步长
    ray_d[np.abs(ray_d) < 1e-5] = 1e-5      # 坐标值小于1e-5的全部=1e-5
    d_intersect = (nominator / ray_d[:, None]).reshape(-1, 6)   # [1024,2,3] / [1024,1,3] = [1024,2,3].resize = d_intersect.shape : [1024, 6]
    
    # 计算6个交点
    # 
    p_intersect = d_intersect[..., None] * ray_d[:, None] + ray_o[:, None]      # [1024,6,1] * [1024,1,3] + [1024,1,3] = p_intersect.shape : (1024, 6, 3)
    
    # calculate the intersections located at the 3d bounding box
    # 计算位于3d bounding box的交点
    ...
    # p_mask_at_box.shape : (1024, 6)       bool

    # obtain the intersections of rays which intersect exactly twice
    # 求出恰好相交两次的射线的交点
    
    mask_at_box = p_mask_at_box.sum(-1) == 2        # 找出相交两次的射线,共1024条       [1024,] bool
    p_intervals = p_intersect[mask_at_box][p_mask_at_box[mask_at_box]].reshape(
        -1, 2, 3)
    # p_intervals.shape : (1024, 2, 3)
    
    # calculate the step of intersections
    ray_o = ray_o[mask_at_box]
    ray_d = ray_d[mask_at_box]
    norm_ray = np.linalg.norm(ray_d, axis=1)        # np.linalg.norm : 1024*[x,y,z] --> 1024*[√(x2)+(y2)+(z2)] L2范式平方和后开方
    d0 = np.linalg.norm(p_intervals[:, 0] - ray_o, axis=1) / norm_ray
    d1 = np.linalg.norm(p_intervals[:, 1] - ray_o, axis=1) / norm_ray
    near = np.minimum(d0, d1)       # 最短距离
...
def sample_ray_h36m(img, msk, K, R, T, bounds, nrays, split):
    ...
    ray_o, ray_d = get_rays(H, W, K, R, T)
    # ray_o.shape : (512, 512, 3)
    # ray_d.shape : (512, 512, 3)
    pose = np.concatenate([R, T], axis=1)   # shape: (3, 4)
    ...
    if split == 'train':
        ...
        while nsampled_rays < nrays:
            n_body = int((nrays - nsampled_rays) * body_sample_ratio)   # 512
            n_face = int((nrays - nsampled_rays) * face_sample_ratio)   # 0
            n_rand = (nrays - nsampled_rays) - n_body - n_face          # 512
            
            # sample rays on body
            coord_body = np.argwhere(msk == 1)      # np.argwhere返回满足条件的位置索引
            

            if len(coord_body) > 0:
                coord_body = coord_body[np.random.randint(0, len(coord_body),
                                                          n_body)]
                # 从0到len(coord_body)随机生成n_body个满足msk==1的索引
            else:
                coord_body = np.ones((1, 2), dtype=np.uint8)
            # coord_body.shape : [n_body, 2]
            ...
            # coord.shape : [n_rand, 2] 

            if len(coord_face) > 0:
                coord = np.concatenate([coord_body, coord_face, coord], axis=0)
            else:
                coord = np.concatenate([coord_body, coord], axis=0)
            # coord.shape : [n_body+n_rand, 2]   [1024, 2]  投射到身体和随机投射的光线在图片上的坐标[x,y]

            ray_o_ = ray_o[coord[:, 0], coord[:, 1]]        # ray_o_.shape: (1024, 3)
            ray_d_ = ray_d[coord[:, 0], coord[:, 1]]        # ray_d_.shape: (1024, 3)
            rgb_ = img[coord[:, 0], coord[:, 1]]            # rgb_.shape:   (1024, 3)
...
def get_acc(coord, msk):        # coord.shape:[1024,2] = [n_body+n_rand,2]      msk.shape:(512,512)
    border = 25
    kernel = np.ones((border, border), np.uint8)
    msk = cv2.dilate(msk.copy(), kernel)
    #print('msk',type(msk))
    #print('coord',type(coord))
    acc = msk[coord[:, 0], coord[:, 1]] # acc.shape: (1024,)
