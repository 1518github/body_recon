...
class Evaluator:
    def __init__(self):
        ...
        if cfg.run_mode == 'train':
            ...
            log_dir = cfg.evaluate_log_dir
        else:
            print('error log direction')
            log_dir = None
        self.writer = SummaryWriter(log_dir=log_dir)
        print('prepare the tensorboard completed and save logs in %s'%(log_dir))
    ...
    def evaluate(self, output, batch, step):
        ...
        self.mse.append(mse)
        #self.writer.add_scalar('mse', mse, step)
        ...
        #self.writer.add_scalar('ssim', ssim, step)
        ...
        print(ssim_str)
    def log(self,step):
        self.writer.add_scalar('mse', np.mean(self.mse), step)
        self.writer.add_scalar('psnr', np.mean(self.psnr), step)
        self.writer.add_scalar('ssim', np.mean(self.ssim), step)
