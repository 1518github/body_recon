...
class SpatialEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.use_first_pool = use_first_pool    # True
        norm_layer = get_norm_layer(norm_type)  # norm_type = batch
        # norm_layer = functools.partial(
        #             nn.BatchNorm2d, affine=True, track_running_stats=True
        #         )
        ...
        self.latent_size = [0, 64, 128, 256, 512, 1024][num_layers]     # 256
        ...
        self.reduction_layer = nn.Conv2d(cfg.img_feat_size, cfg.embed_size,1)   # (256, 64, 1)
        self.num_layers = num_layers
        self.index_interp = index_interp        # bilinear
        self.index_padding = index_padding      # zeros
        self.upsample_interp = upsample_interp  # bilinear
    ...
    def forward(self, x):
        ...
        if self.use_custom_resnet:
            ...
            # x.shape: torch.Size([3, 3, 512, 512])
            x = self.model.conv1(x)
            # x.shape: torch.Size([3, 64, 256, 256])
            x = self.model.bn1(x)
            # x.shape: torch.Size([3, 64, 256, 256])
            x = self.model.relu(x)
            ...
            latent_sz = latents[0].shape[-2:]   # 256,256
            # print(len(latents))       = 3
            # 1 : torch.Size([3, 64, 256, 256])
            # 2 : torch.Size([3, 64, 128, 128])
            # 3 : torch.Size([3, 128, 64, 64])
            for i in range(len(latents)):
                latents[i] = F.interpolate(     # 插值采样
                    latents[i],     # input_size
                    latent_sz,      # output_size
                    mode=self.upsample_interp,  # 双线性插值
                    align_corners=align_corners,    # 像素对齐
                )

            pixel_feat_map = torch.cat(latents, dim=1)  # torch.Size([3, 256, 256, 256])


        pixel_feat_scale = np.array(
            [pixel_feat_map.shape[-1], pixel_feat_map.shape[-2]])
        pixel_feat_scale = pixel_feat_scale / (pixel_feat_scale - 1) * 2.0  # 256/255 = [2.00784314 2.00784314]

        holder_feat_map = self.reduction_layer(pixel_feat_map)  # conv(256->64,kernel=1) : torch.Size([3, 64, 256, 256])
        holder_feat_scale = np.array(
            [holder_feat_map.shape[-1], holder_feat_map.shape[-2]])
        holder_feat_scale = holder_feat_scale / (
                    holder_feat_scale - 1) * 2.0        # [2.00784314 2.00784314]
